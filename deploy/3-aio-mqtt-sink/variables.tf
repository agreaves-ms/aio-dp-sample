...
variable "mqtt_sink_image" {
  description = "(Optional) The MqttSink image name. (Otherwise, 'mqttsink')"
  type        = string
  default     = "mqttsink"
}

variable "mqtt_sink_version" {
  description = "(Optional) The MqttSink image version. (Otherwise, '0.0.1')"
  type        = string
  default     = "0.0.1"
}
